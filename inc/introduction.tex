...
* MACS Students
* Unity Developers/Game Developers
* Those with interest in RDW

\section{Thesis Structure}
The thesis itself has a somewhat nested structure which corresponds to each research question. Each research question has an experiment dedicated to it which has its own method, result and discussion components. The reasoning for this structure is that parts of the results for the experiment on noticability are necessary for the second experiment which focuses on effectiveness. As such, it would be preferable to present the first experiment before moving on to the second. The overall structure of this thesis is as follows:

\begin{enumerate}
    \item Introduction
    \item Related Work and Background
    \item Methods for the Thesis as a Whole
    \item Implementation for Components Used in Both Experiments
    \item Experiment 1: Measuring the Noticability of Distractors
    \begin{enumerate}
